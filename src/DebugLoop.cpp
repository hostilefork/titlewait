...
BOOL PreviousStillRunning(std::wstring & exeImageName, DWORD processId)
{
    ...
    for (int index = 0; index < dwPIDCount; index++) {
        ...
        if (
			0 == lstrcmpi(
				mainDebuggeeProcessInfo->szImgPath,
				lpi[index].szImgPath
			)
		){
...
DWORD WINAPI DebugLoopMain(LPVOID lpParam) // returns a MainReturn
{
    ...
    while (TRUE) {
        ...
        switch (DebugEv->dwDebugEventCode) {
          case EXCEPTION_DEBUG_EVENT: {
            ...
            // (dwContinueStatus). This value is used by the
            ...
            switch (DebugEv->u.Exception.ExceptionRecord.ExceptionCode) {
              ...
              case EXCEPTION_BREAKPOINT: {
                //
                // First chance: Display the current instruction and
                // register values.
                //
                debugInfo(L"EXCEPTION_BREAKPOINT");

                // No process-functions during CREATE_PROCESS_DEBUG_EVENT.
                // We have to wait for a later time.
                //
                if (not config->defer or previouslyWaited or suspendedThread)
                    break;
              ...
              case EXCEPTION_SINGLE_STEP:
                //
                // First chance: Update the display of the current instruction
                // and register values.
                //
              ...
              default:
                ...
                // We have debugger behaviors for everything we understand, but
                // not random exceptions...pass on up the chain and say we
                // don't handle it.
                //
                handledEvent = false;
                break;
          ...
          case CREATE_THREAD_DEBUG_EVENT: {
            ...
            // GetThreadContext() and SetThreadContext() functions, and suspend
            // and resume thread execution with the SuspendThread() and
          ...
          case EXIT_PROCESS_DEBUG_EVENT: {
            ...
            //
            // We used to exit here, but now we wait for the LAST process
            // exiting signal.  We could in theory return ClosedReturn if
            // we want the behavior of quitting just because the main
            // process we spawned quit...but that is not a good default
            // when people are using things like iexplore which launches a
            // child.... so make that a non-default option?
            //
            if (
                WaitForSingleObject(lastProcessExitedEvent, INFINITE)
                != WAIT_OBJECT_0
            ){
                WindowsVerify(L"WaitForSingleObject", FALSE);
            }
    ...
    return TitleWait::InternalErrorReturn;  // unreachable point
}
