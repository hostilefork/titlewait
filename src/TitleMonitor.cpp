...
BOOL CALLBACK EnumTopLevelDesktopWindowsProc(HWND topLevelWindow, LPARAM lparam)
{
    ...
    if (not config->searchAll) {
        ...
        if (not windowIsInSpawnedProcess)
            return TRUE;
        ...
        //
        if (config->shouldMoveWindow() and not movedWindow) {
    ...
    //
    if (not title.empty() and not config->regex.empty()) {
        std::wsmatch what;
...
DWORD WINAPI TitleMonitorThreadProc(LPVOID lpParam)
{
    ...
    // Initialize a timer that will be called every N seconds to see if window
    // has the title regex we are looking for.
    //
    ...
    // WaitOnSingleObject for this thread handle blocks until value returned
